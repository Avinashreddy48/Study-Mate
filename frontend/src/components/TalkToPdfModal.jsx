  ...
  const handleQuery = async (queryText) => {
    ...
    try {
      ...
      const aiMessage = {
        ...
        pageSpecific: data.page_specific || false,
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {conversation.length === 0 && (
            <div className="text-center text-gray-500 py-8">
              ...
              <p className="mb-4">Click the microphone button below to start asking questions about your documents!</p>

              <div className="text-left bg-blue-50 p-4 rounded-lg border border-blue-200 max-w-md mx-auto">
                <h4 className="font-medium text-blue-800 mb-2">💡 Try asking:</h4>
                <ul className="text-sm text-blue-700 space-y-1">
                  <li>• "What's on page 5?"</li>
                  <li>• "Summarize pages 3 to 7"</li>
                  <li>• "What does page 10 say about..."</li>
                  <li>• "Tell me about the introduction"</li>
          ...
          {conversation.map((message, index) => (
            ...
            >
              ...
              >
                ...
                {/* Show page information for AI responses */}
                {message.type === 'ai' && message.pageSpecific && message.pagesFound && message.pagesFound.length > 0 && (
                  <div className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded mt-2 border border-blue-200">
                    📄 Found on page{message.pagesFound.length > 1 ? 's' : ''}: {message.pagesFound.join(', ')}
                  </div>
                )}

                {/* Show relevant sections count */}
                {message.type === 'ai' && message.relevantSections && message.relevantSections.length > 0 && (
                  <div className="text-xs bg-green-50 text-green-700 px-2 py-1 rounded mt-1 border border-green-200">
                    📚 Found {message.relevantSections.length} relevant section{message.relevantSections.length > 1 ? 's' : ''}
                  </div>
                )}
