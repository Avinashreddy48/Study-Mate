...
async def simple_text_search(selected_text: str):
    ...
    try:
        ...
        doc_ids = list(set(section.get("document_id") for section in all_sections if section.get("document_id")))
        ...
        for doc_id in doc_ids:
            try:
                # Try to convert to ObjectId (for old format)
                object_ids.append(ObjectId(doc_id))
            except:
                # If it fails, it's probably a UUID (new format)
                uuid_ids.append(doc_id)

        # Query for both types of IDs
        documents = []
        if object_ids:
            docs_by_objectid = await mongo_db.db["documents"].find({"_id": {"$in": object_ids}}).to_list(length=None)
            documents.extend(docs_by_objectid)
        if uuid_ids:
            docs_by_uuid = await mongo_db.db["documents"].find({"_id": {"$in": uuid_ids}}).to_list(length=None)
            documents.extend(docs_by_uuid)

        document_info = {str(doc["_id"]): doc for doc in documents}

        # Simple keyword-based matching
        search_words = set(re.findall(r'\b\w+\b', selected_text.lower()))
        matching_sections = []
        ...
        # Create snippets from top matches
        snippets = []
        for item in matching_sections[:10]:  # Top 10 matches
            section = item["section"]
            content = section.get("content", "")

            # Extract relevant sentences containing search terms
            ...
            for sentence in sentences:
                ...
                if len(sentence) > 20:
                    ...
                    if search_words.intersection(sentence_words):
                        relevant_sentences.append(sentence)

            # Take best sentences or fallback to first part of content
            if relevant_sentences:
                snippet_text = ". ".join(relevant_sentences[:2])
            else:
                snippet_text = content[:200] + "..." if len(content) > 200 else content
            ...
            snippets.append({
                ...
                "similarity": item["relevance"]  # Use relevance as similarity
            })
        ...
        print(f"Error in simple text search: {e}")
        return {
            "snippets": [],
            "selected_text": selected_text,
            "contradictions": [],
...
async def semantic_search(request: dict):
    ...
    try:
        ...
        from services.recommendation_engine import HAS_SENTENCE_TRANSFORMERS
        if not HAS_SENTENCE_TRANSFORMERS:
            # Fallback to simple text-based search
            return await simple_text_search(selected_text)
...
async def upload_document_cluster(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        all_sections, all_pages, texts_to_embed, doc_records = [], [], [], []

        for path, result in extraction_results.items():
            sections = result.get("sections", [])
            bookmarks = result.get("bookmarks", [])
            pages = result.get("pages", [])  # Get page content
        ...
        if all_pages: await mongo_db.db["pages"].insert_many(all_pages)
    finally:
